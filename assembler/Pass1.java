...
public class Pass1 {
	...
	private String[] directives = {"WORD" , "BYTE", "RESW", "RESB", "EQU", "ORG","START","END"};
	boolean check = true;
	...
	private void line(String line) {
		...
		if (firstChar == '.') {
			...
			if (!empty) {
				...
				if(operationCode=="END"||operationCode=="LTORG"){
					while(latableindx!=latable.size()){
						labels.add("*");
						operationCodes.add(" ");
						operands.add(latable.get(latableindx));
						latableindx++;
					}
				}
				...
				if(operand.charAt(0)=='='&&operand.charAt(2)=='\''&&operand.charAt(operand.length()-1)=='\''){
	...
	private void createAddressArray() {
		...
		for (int i = 1; i < labels.size(); i++) {
			if (operationCodes.get(i).equals("WORD")) {
				Address += 3;
	...
	void run() {
		...
		if(check){
		Pass2 g = new Pass2(labels, operationCodes, operands, Addresses, symTabName, symTabAddress,latable,latableAdd, objFile, txtFile);
		}
	}
